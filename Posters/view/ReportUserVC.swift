...
class ReportUserVC: UIViewController {
    ...
    @IBAction func submitBtnTapped(_ sender: Any) {
        if edt_subject.text == ""{
            self.view.makeToast("Please enter the reason.", duration: 2.5, position: .center, title: nil, image: nil, style: .init()) { didTap in
            }
            return
        }
        if txt_desc.text == ""{
        ...
        SVProgressHUD.show()
        DataManager.shared.report_user(subject: edt_subject.text ?? "", message: txt_desc.text, related_group_id: relatedGroupID, opp_id: oppID){ success, message in
